...
print(f"Akcja: {action} | ID: {folder_id} | Nazwa: {folder_name} | Nowa nazwa: {new_name}")
...
try:
    ...
    print(f"BLAD KV GET → HTTP {e.code}: {e.read().decode()}")
    sys.exit(1)
...
if action == 'add':
    ...
    f = next((f for f in folders if f['id'] == folder_id), None)
    if not f:
        print(f"Nie znaleziono folderu: {folder_id}")
        sys.exit(1)
    old_name = f['name']
    f['name'] = new_name
    print(f"Zmieniam nazwe: '{old_name}' → '{new_name}'")
